...
- **引导学习助理（内置）** —— 一位懂得循序渐进的导师：先了解你想学什么，给出带多级编号
  ...
  之后给出总结，列出需要加强的部分。**你说什么语言，它就用什么语言教。** 详细设定见
  [内置助理](#内置助理引导学习)。
...
![画一张图，然后在图表面板里打开它，再切到脉络看自动归类](docs/assets/diagram-and-threads.zh.gif)
...
它的说话方式是深度、实战、最佳实践导向的，不是入门简介。**用哪种语言由你决定**：你用中文它就
用中文，你用英文它就用英文，中途换语言它也跟着换——计划、清单、题目和总结都跟着换。

遇到专业术语，它会在第一次出现时用括号给出原文（如果原文是另一种语言），避免翻译带来的
歧义。
...
其中 `<数据文件夹>/backups/` 是**升级数据库前自动留下的快照**（最近 5 份）。数据库结构变化时，
...
**应用本身需要你动手换一次**：面板上会提示有新版本，点一下去下载新的安装包，覆盖安装即可——
你的数据在数据文件夹里，覆盖安装不会碰它。这里没有做"一键自动更新"，因为 macOS 上的自动更新
要求应用有付费的代码签名，而这个项目没有购买证书（见上面的安装提示）。三个平台用同一套做法，
比一个只在两个平台能用的按钮更诚实。
