...
const DESCRIPTION = [
  ...
  "",
  `Use it when there are ${ASK_USER_MIN_OPTIONS}–${ASK_USER_MAX_OPTIONS} defensible ways forward and the choice changes what you build, when a detail you cannot infer would otherwise be guessed at, or when you have just produced a plan, a design or another substantial artifact and the user's approval decides what happens next. Ask 1–${ASK_USER_MAX_QUESTIONS} questions in one call; they are presented one at a time as tabs with a single submit at the end.`,
  "",
  "Rules:",
  "- When your instructions say to wait for the user's confirmation or to let them choose before continuing, that is this tool. Ask here rather than ending your message with a question in prose — a question written as plain text gets a free-text answer, and a choice needs options.",
  ...
  "- That does not reach the user's approval of your work: nothing in the conversation can supply it, so asking to confirm an artifact is never a redundant question.",
