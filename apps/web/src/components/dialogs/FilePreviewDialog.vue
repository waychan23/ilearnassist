 ...
 *
 * Three renderings, chosen by `kind` from the server rather than by the extension here: text
 * is syntax-highlighted into a `<pre>`, Markdown goes through the same renderer the messages
 * use (so `html: false` is what keeps a file from injecting markup), and anything else gets
 * the file's name and size with a note that the format is not previewable yet. That last
 * branch is the whole extension point — a new format is a new `kind` and a new branch.
 *
...
/** Which of Markdown's two views is showing. Reset for each file; see the watcher below. */
const markdownView = ref<"rendered" | "source">("rendered");
    ...
    >
      <div class="modal lg">
        <div class="modal-head">
          ...
          <!-- Markdown only: it is the one format with two things to look at. A segmented
               control rather than two buttons, because it is one choice — see `.segmented` in
        ...
        <div class="modal-body file-body" data-testid="file-preview-body">
          <p v-if="store.fileContentLoading" class="file-note" data-testid="file-preview-loading">
          ...
          <p v-else-if="store.filePreviewError" class="file-note error" data-testid="file-preview-error">
            {{ store.filePreviewError }}
          </p>

          <div
            v-else-if="content && content.kind === 'unsupported'"
          ...
          <div
            v-else-if="content && content.kind === 'markdown' && markdownView === 'rendered'"
            class="markdown"
            v-html="rendered"
          ></div>
          ...
          <pre v-else-if="content" class="hljs file-text" data-testid="file-preview-text"><code v-html="highlighted"></code></pre>
